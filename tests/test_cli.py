...
from tests.conftest import VALID_TMS_XML, INVERTED_ZOOM_XML, MAX_ZOOM_23_XML
...
class TestBasicValidation:
    ...
    def test_multiple_valid_files_exits_zero(self, tmp_path):
        ...
        _write_xml(tmp_path, VALID_TMS_XML.replace("Test TMS Map", "Map B"), "map_b.xml")
        code = _run_main(str(tmp_path))
        assert code == 0
...
class TestIssuesFlag:
    ...
    @patch("mapvalidator.__main__.manage_github_issues")
    @patch("mapvalidator.__main__.probe_all", return_value=[])
    def test_issues_with_empty_probes_skips_manage(self, mock_probe, mock_issues, tmp_path):
...
class TestDefaultDirectory:
    ...
    def test_defaults_to_cwd(self, mock_report, mock_dup, mock_corpus, tmp_path, monkeypatch):
